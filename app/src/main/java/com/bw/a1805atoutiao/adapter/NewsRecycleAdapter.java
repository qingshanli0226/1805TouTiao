...
import com.bw.framework.glide.GlideImage;
...
public class NewsRecycleAdapter extends BaseRVAdapter<News> {
    ...
    protected void convert(News itemData, BaseViewHolder baseViewHolder, int position) {
        ...
        if (itemData.getAvatar_url()!=null){
            GlideImage.circleImage(baseViewHolder.itemView.getContext(),itemData.getAvatar_url(),ivMedia);
        }

        if (itemData.getUrl()!=null){
            GlideImage.Image(baseViewHolder.itemView.getContext(),itemData.getUrl(),ivImage);
        }
