package com.song.picture.binder;
...
import com.song.picture.widget.CircleImageView;
    ...
    protected void onBindViewHolder(@NonNull final NewsArticleVideoViewBinder.ViewHolder holder, @NonNull final MultiNewsArticleDataBean item) {
        ...
        try {
            if (null != item.getVideo_detail_info()) {
                if (null != item.getVideo_detail_info().getDetail_video_large_image()) {
                    ...
                    if (!TextUtils.isEmpty(image)) {
                        ImageLoader.loadCenterCrop(context, image, holder.iv_video_image, R.color.viewBackground, R.drawable.error_image);
                    }
                }
            } else {
                holder.iv_video_image.setImageResource(R.drawable.error_image);
            }
            ...
            if (!TextUtils.isEmpty(tv_datetime)) {
                tv_datetime = TimeUtil.getInstance().getTimeStampAgo(tv_datetime);
            }
