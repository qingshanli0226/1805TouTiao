...
import com.song.common.SdkManager;
import com.song.fromwork.utils.InitApp;
import com.song.fromwork.utils.NetWorkUtil;
...
public class RetrofitFactory {
    ...
    private static final Interceptor cacheControlInterceptor = chain -> {
        ...
        if (!NetWorkUtil.isNetworkConnected(InitApp.getAppContext())) {
    ...
    public static Retrofit getRetrofit() {
        if (retrofit == null) {
            synchronized (RetrofitFactory.class) {
                if (retrofit == null) {
                    // 指定缓存路径,缓存大小 50Mb
                    Cache cache = new Cache(new File(InitApp.getAppContext().getCacheDir(), "HttpCache"),
                            1024 * 1024 * 50);

                    // Cookie 持久化
                    ClearableCookieJar cookieJar =
                            new PersistentCookieJar(new SetCookieCache(), new SharedPrefsCookiePersistor(InitApp.getAppContext()));
